  ...
  return (
    <section className="page">
      ...
      <div className="chips filter-chips">
        <button
          className={`chip${hubFilter === 'all' ? ' chip-active' : ''}`}
        ...
        {HUBS.map(h => (
          <button
            key={h.slug}
            className={`chip${hubFilter === h.slug ? ' chip-active' : ''}`}
            onClick={() => setHubFilter(h.slug)}
          >
            {h.title}
          </button>
        ))}
      ...
      {items.length === 0 ? (
        ...
        <div className="res-grid">
          {items.map(i => {
            ...
            return (
              <Link key={`${i.hubSlug}-${i.slug}`} to={`/resource/${i.slug}`} className="res-card">
